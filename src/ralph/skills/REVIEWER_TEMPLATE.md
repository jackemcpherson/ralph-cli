...
<!--
  ...
  - Feature branch changes (diff vs main) plus uncommitted changes
  - Specific file patterns to include/exclude?
  - Note fallback behavior when on main branch
-->

Review all files changed on the feature branch plus any uncommitted changes.
...
> **Note**: Filter results to relevant file patterns (e.g., `-- '*.py'` for Python files).
